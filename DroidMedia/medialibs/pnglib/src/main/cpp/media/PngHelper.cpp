...
PngHelper::PngHelper(const string &file_name) :
    ...
    infop->row_pointers;
    mWidth = png_get_image_width(png, infop); // 1920
    mHeight = png_get_image_height(png, infop); // 1080
    mColorType = png_get_color_type(png, infop); // PNG_COLOR_TYPE_RGB 2
    mBitDepth = png_get_bit_depth(png, infop); // 8
    ...
    unsigned int row_bytes = png_get_rowbytes(png, infop); // 5760 = 1920 * 3
    if (mPixelData != nullptr) {
        delete[] mPixelData;
        mPixelData = nullptr;
    }
    mPixelData = new unsigned char[row_bytes * mHeight];
    sizeOfPixels = row_bytes *mHeight;
