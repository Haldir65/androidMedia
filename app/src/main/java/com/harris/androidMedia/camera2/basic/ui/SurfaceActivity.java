...
public class SurfaceActivity extends AppCompatActivity implements View.OnClickListener, ActivityCompat.OnRequestPermissionsResultCallback{
    ...
    private void openCamera(int width, int height) {
        if (ContextCompat.checkSelfPermission(SurfaceActivity.this, Manifest.permission.CAMERA)
                != PackageManager.PERMISSION_GRANTED) {
            //requestCameraPermission();
            Toast.makeText(SurfaceActivity.this, "打开相机失败！", Toast.LENGTH_SHORT).show();
            return;
        }
     ...
     * 设置所有相机的参数，包括前置后置
     * 设置的参数包括人脸检测，该设备所能支持的最大图片分辨率
     *
     * @param width  The width of available size for camera preview 预览框的宽度
     * @param height The height of available size for camera preview 预览框的高度
     */
     ...
     * 创建一个CaptureSession，一个Session里面可以连续发起多个request,
     * session创建完毕 onConfigured之后使用Builder创建Request，并且将request交给session处理
     * 预览阶段使用setRepeatingRequest
     * 整体是使用Builder创建一个Request对象，再通过 mCaptureSession.setRepeatingRequest 将这个request请求发送给camera device
     */
     ...
     * finished. 取消对焦锁定状态，开始repeating request 发送
     */
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.picture:
                ...
                break;
            case R.id.album:
                Toast.makeText(SurfaceActivity.this, "跳转至文件浏览器", Toast.LENGTH_SHORT).show();
